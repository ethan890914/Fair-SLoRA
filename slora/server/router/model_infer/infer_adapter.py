...
class InferAdapter:
    ...
    # Create none slot in adapters
    def ensure_none_slot(self):
        # If none slot already exists, do nothing
        if self.none_adapter_id >= 0:
            return

        # none id is the next index after all real adapters currently loaded
        self.none_adapter_id = len(self.adapter_dirs)

        # Make it visible in id space
        self.adapter_dirs.append("__none__")
        self.idx_map["__none__"] = self.none_adapter_id

        # Append rank-0 metadata
        self.a_start = torch.cat([self.a_start, torch.tensor([0], dtype=torch.long, device="cuda")])
        self.a_len = torch.cat([self.a_len, torch.tensor([0], dtype=torch.long, device="cuda")])
        self.a_scaling = torch.cat([self.a_scaling, torch.tensor([1.0], dtype=torch.float16, device="cuda")])

        # No need to change a_loc, because len=0 means no entries are accessed
    # Helper
    def _pop_none_slot(self):
        if self.none_adapter_id < 0:
            return
        # remove last entry (should be none)
        assert self.adapter_dirs[-1] == "__none__"
        self.adapter_dirs.pop()
        self.idx_map.pop("__none__")
        self.a_start = self.a_start[:-1]
        self.a_len = self.a_len[:-1]
        self.a_scaling = self.a_scaling[:-1]
        self.none_adapter_id = -1

    def _push_none_slot(self):
        self.ensure_none_slot()
